...
to an area extent defined (for now) for the ase  experiment.
...
path_measures_comp = os.path.join(MAIN_PATH, config['input_files']['measures_comp'])
...
# Crop velocity data to the ase  extend
...
# Crop velocity data from MEaSUREs to the ase  extend
vxm_s = vel_tools.crop_velocity_data_to_extend(vx_m, ase_bbox, return_xarray=True)
vym_s = vel_tools.crop_velocity_data_to_extend(vy_m, ase_bbox, return_xarray=True)
stdm_vx_s = vel_tools.crop_velocity_data_to_extend(std_vx_m, ase_bbox, return_xarray=True)
stdm_vy_s = vel_tools.crop_velocity_data_to_extend(std_vy_m, ase_bbox, return_xarray=True)

# Crop velocity data from Itslive interpolated to the ase  extend
vxi_s = vel_tools.crop_velocity_data_to_extend(vx_i, ase_bbox, return_xarray=True)
vyi_s = vel_tools.crop_velocity_data_to_extend(vy_i, ase_bbox, return_xarray=True)
