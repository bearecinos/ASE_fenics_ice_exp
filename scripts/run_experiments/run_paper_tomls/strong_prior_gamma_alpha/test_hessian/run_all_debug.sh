...
export run_inv_output_dir=$OUTPUT_DIR/10_test_gamma_alpha_10000
...
path_logs=$RUN_CONFIG_DIR/run_paper_tomls/strong_prior_gamma_alpha/test_hessian
...
mpirun -n $1 python $FENICS_ICE_BASE_DIR/runs/run_forward.py $2 |& tee $path_logs/log_toml_fwd_one.txt
OUT=$(tail "$path_logs/log_toml_fwd_one.txt")
echo $OUT | mail -s "run fwd finish config 1" beatriz.recinos@ed.ac.uk

#echo $(date -u) "Run inversion stages started" | mail -s "run inv started" beatriz.recinos@ed.ac.uk
#mpirun -n $1 python $FENICS_ICE_BASE_DIR/runs/run_inv.py $3 |& tee $path_logs/log_toml_inv_two.txt
#OUT=$(tail "$path_logs/log_toml_inv_two.txt")
#echo $OUT | mail -s "run inv finish config 2" beatriz.recinos@ed.ac.uk

#echo $(date -u) "Run forward stages started" | mail -s "run fwd started" beatriz.recinos@ed.ac.uk
#mpirun -n $1 python $FENICS_ICE_BASE_DIR/runs/run_forward.py $3 |& tee $path_logs/log_toml_fwd_two.txt
