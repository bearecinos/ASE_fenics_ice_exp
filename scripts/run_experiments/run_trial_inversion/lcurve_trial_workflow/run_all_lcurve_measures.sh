...
export run_inv_output_dir=$OUTPUT_DIR/03_trial_lcurves_exp
...
path_logs=$RUN_CONFIG_DIR/run_trial_inversion/lcurve_trial_workflow
...
echo $(date -u) "Run inversion stages started measures" | mail -s "run inv lcurve started" beatriz.recinos@ed.ac.uk
mpirun -n $1 python $FENICS_ICE_BASE_DIR/runs/run_inv.py $2 |& tee $path_logs/log_tom4_inv.txt
OUT=$(tail "$path_logs/log_tom4_inv.txt")
echo $OUT | mail -s "run inv finish config measures" beatriz.recinos@ed.ac.uk
